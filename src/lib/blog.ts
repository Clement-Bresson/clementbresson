...
function parseId(id: string): { slug: string; locale: Locale } {
  ...
  if (!slug || !locale || !(locales as readonly string[]).includes(locale)) {
    throw new Error(`Unexpected blog entry id "${id}". Expected "<slug>/<en|fr>".`);
  }
...
export function getAllPosts(): Promise<Post[]> {
  cache ??= (async () => {
    const entries = await getCollection('blog', (e) => includeDrafts || !e.data.draft);
    ...
    const incomplete = [...bySlug]
      ...
      .map(([slug, langs]) => `${slug} (missing: ${locales.filter((l) => !langs.has(l)).join(', ')})`);
    ...
    if (mismatched.length) {
      throw new Error(`Both languages of an article must have the same tags. Mismatched: ${mismatched.join(', ')}`);
    }
...
export async function getTags(locale: Locale): Promise<TagSummary[]> {
  ...
  return tagIds
    .map((id) => ({ id, ...tagCopy(id, locale), posts: posts.filter((p) => p.data.tags.includes(id)) }))
...
export async function getRelated(post: Post, limit = 3): Promise<Post[]> {
  const others = (await getPosts(post.locale)).filter((p) => p.slug !== post.slug);
  const shared = (p: Post) => p.data.tags.filter((t) => post.data.tags.includes(t)).length;
  return others
...
export const fileUrl = (locale: Locale, file: string) => getAbsoluteLocaleUrl(locale, file).replace(/\/$/, '');
export const filePath = (locale: Locale, file: string) => getRelativeLocaleUrl(locale, file).replace(/\/$/, '');
