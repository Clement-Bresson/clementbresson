...
import { blogPath, postPath, tagPath, type Post, type TagSummary } from './blog';

type Node = Record<string, unknown>;

const tagLabel = (id: TagId, locale: Locale) => tagCopy(id, locale).label;

const siteUrl = (site: URL | undefined) => (site ?? new URL('https://clementbresson.com')).origin + '/';
...
export function personNode(site: URL | undefined, locale: Locale, portraitUrl: string): Node {
  ...
  return {
    ...
    knowsAbout: tagIds.filter((id) => id !== 'site').map((id) => tagLabel(id, locale)),
    sameAs,
  };
}
...
export function blogNode(site: URL | undefined, locale: Locale, posts: Post[]): Node {
  const t = getCopy(locale);
...
export function tagPageNode(site: URL | undefined, locale: Locale, tag: TagSummary): Node {
  const t = getCopy(locale);
...
export function blogPostingNode(
  site: URL | undefined,
  post: Post,
  opts: { coverUrl?: string; fallbackImageUrl: string; wordCount: number; readingMinutes: number },
): Node {
  ...
  return {
    ...
    ...(post.data.tags.length
      ? { keywords: post.data.tags.map((id) => tagLabel(id, post.locale)).join(', ') }
      : {}),
    ...(post.data.tags.length ? { articleSection: tagLabel(post.data.tags[0], post.locale) } : {}),
