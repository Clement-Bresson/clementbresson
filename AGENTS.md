...
- Faithful, not enriched: the article says what the text says, in both languages. Never invent a number, quote or person. Sources are researched, not guessed: every product, company, person, talk, book or article the text names gets its official page found online and fetched, then a `sources` entry with `url` and a link on the first mention in both bodies. When the text states a figure, a study or a claim that needs a source and none can be found, keep the sentence and add `<!-- TODO source: what is missing -->` on its own line right after the paragraph, in both `fr.md` and `en.md`. Use a Markdown comparison table when the text compares alternatives; do not force one.
...
- `linkedin`: URL of the original LinkedIn post when the article started there (rendered as attribution and as schema `sameAs`). `sources`: list of `{ title, author?, year?, url? }` the article cites, rendered as a Sources section and as schema `citation`. Every URL is verified by fetching it; none is guessed.
...
- `updatedDate` is derived from git (`src/lib/modified-date.ts`: day of the last commit touching the language file, after a baseline commit, only when later than `pubDate`); the frontmatter field overrides it and is otherwise left unset. Editing an article needs no date bookkeeping.
