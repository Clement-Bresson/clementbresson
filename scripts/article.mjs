...
function validateSpec(spec, knownTags) {
  ...
  if (
    !/^\d{4}-\d{2}-\d{2}$/.test(spec.pubDate ?? "") ||
    Number.isNaN(Date.parse(spec.pubDate))
  )
    errors.push("pubDate must be YYYY-MM-DD");
  if (
    spec.updatedDate &&
    (!/^\d{4}-\d{2}-\d{2}$/.test(spec.updatedDate) ||
      spec.updatedDate < spec.pubDate)
  )
  ...
  for (const t of spec.tags ?? [])
    if (!knownTags.includes(t))
      errors.push(
        `unknown tag "${t}" (known: ${knownTags.join(", ")}). Add it to src/data/tags.ts first.`,
      );
  if (
    spec.linkedin &&
  ...
  for (const locale of LOCALES) {
    ...
    if (!l.body?.trim()) errors.push(`${locale}.body is required`);
      ...
      if (/^#{2,6} +sources?\s*$/im.test(l.body))
        errors.push(
          `${locale}.body must not contain a Sources heading: use the "sources" field`,
        );
      if (
        /(n'hésitez pas à me suivre|DM ouverts|MP ouverts|follow me|DMs? are open)/i.test(
          l.body,
        )
      )
        warn(`${locale}.body still contains LinkedIn call-to-action text`);
      if (/\]\(https?:\/\/(www\.)?clementbresson\.com/.test(l.body))
        errors.push(
          `${locale}.body links to the site with an absolute URL: use /blog/<slug>/ or /fr/blog/<slug>/`,
        );
      const wrongPrefix =
        locale === "fr" ? /\]\(\/blog\//g : /\]\(\/fr\/blog\//g;
      if (wrongPrefix.test(l.body))
        errors.push(`${locale}.body links to the other language's blog path`);
    }
  ...
  if (spec.fr && spec.en && spec.fr.body && spec.en.body) {
    ...
    for (const [name, re] of [
      ...
      if (count(spec.fr.body, re) !== count(spec.en.body, re))
        warn(`fr and en bodies differ in number of ${name}`);
    }
  }
  return errors;
}
...
async function create(specPath, flags) {
  ...
  for (const locale of LOCALES) {
    for (const m of spec[locale].body.matchAll(
      /!\[[^\]]*\]\(\.\/([^)\s]+)\)/g,
    )) {
      if (!placed.has(m[1]))
        fail(
  ...
  ok(
    `wrote ${path.relative(ROOT, dir)}/{fr,en}.md${placed.size ? ` + ${placed.size} image(s)` : ""}`,
  );
  console.log(
    `  EN https://clementbresson.com/blog/${spec.slug}/\n  FR https://clementbresson.com/fr/blog/${spec.slug}/`,
  );
...
async function check(slug) {
  ...
  const slugs = slug
    ? [slug]
    : (await readdir(BLOG, { withFileTypes: true }))
  ...
  for (const s of slugs) {
    ...
    if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(s))
      report("folder name is not kebab-case");
    const fm = {};
    for (const locale of LOCALES) {
      ...
      const m = src.match(/^---\n([\s\S]*?)\n---\n([\s\S]*)$/);
      if (!m) {
        report(`${locale}.md has no frontmatter`);
        continue;
      }
      const [, head, body] = m;
      const get = (k) =>
        head.match(new RegExp(`^${k}: (.*)$`, "m"))?.[1]?.trim();
      const tags = [...(get("tags") ?? "").matchAll(/'([a-z0-9-]+)'/g)].map(
        (x) => x[1],
      );
      ...
      for (const im of body.matchAll(/!\[[^\]]*\]\(\.\/([^)\s]+)\)/g))
        if (!existsSync(path.join(dir, im[1])))
          report(`${locale}.md references missing image ./${im[1]}`);
      const cover = get("cover");
      if (cover && !existsSync(path.join(dir, cover.replace(/^\.\//, ""))))
        report(`${locale}.md cover ${cover} missing`);
      const wrongPrefix =
        locale === "fr" ? /\]\(\/blog\//g : /\]\(\/fr\/blog\//g;
      if (wrongPrefix.test(body))
        report(`${locale}.md links to the other language's blog path`);
      for (const link of body.matchAll(
        /\]\(\/(?:fr\/)?blog\/([a-z0-9-]+)\/\)/g,
      )) {
        if (!existsSync(path.join(BLOG, link[1])))
          report(`${locale}.md links to unknown article ${link[1]}`);
      }
    }
    if (fm.fr && fm.en) {
      if (fm.fr.tags.join() !== fm.en.tags.join())
        report("tags differ between fr.md and en.md");
      if (fm.fr.pubDate !== fm.en.pubDate)
        report("pubDate differs between fr.md and en.md");
    }
    const files = await readdir(dir);
    for (const f of files) {
      if (IMAGE_EXT.test(f)) {
        const used =
          LOCALES.some((l) => existsSync(path.join(dir, `${l}.md`))) &&
          (
            await Promise.all(
              LOCALES.map(
                async (l) =>
                  existsSync(path.join(dir, `${l}.md`)) &&
                  (await readFile(path.join(dir, `${l}.md`), "utf8")).includes(
                    f,
                  ),
              ),
            )
...
async function index() {
  ...
  for (const s of slugs) {
    ...
    for (const locale of LOCALES)
      if (row[locale])
        console.log(
          `  ${locale.toUpperCase()} ${row[locale].title}\n     ${row[locale].description}`,
        );
    if (row.headings?.length)
      console.log(`  sections: ${row.headings.join(" · ")}`);
  }
}

const [cmd, arg, ...rest] = process.argv.slice(2);
const flags = {
  build: rest.includes("--build") || arg === "--build",
  force: rest.includes("--force") || arg === "--force",
};
if (cmd === "create" && arg && !arg.startsWith("--")) await create(arg, flags);
else if (cmd === "check")
  await check(arg && !arg.startsWith("--") ? arg : undefined);
