...
6. **Attach the domain**: uncomment the `routes` block in `wrangler.jsonc` and push. Cloudflare creates the DNS records and TLS certificate for `clementbresson.com` and `www.clementbresson.com`. `public/_redirects` sends `www` to the apex.
